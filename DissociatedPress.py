...
def dissociated_press_from_file(fname, chunk_size, chunks):
    """Run the Dissociated Press algorithm on the text in the given file.
...
def dissociated_press(text, chunk_size, chunks):
    ...
    words = filter(None, text_clean.split(" "))
...
def dissociated_press_iterate(words, last_chunk, chunk_size = -1):
    ...
    size = len(last_chunk)
    matches = find_in_list_consecutive(words, last_chunk)
    # Pick a random match.
    start = random.choice(matches)
    return words[start + size:start + size*2], len(matches)

def find_in_list_consecutive(l, pattern):
    """Find a pattern inside of a list. Returns a list of indices to 'l' where
    'pattern' is found.
        ...
        try:
            while True:
                ...
                while all_matched and i < len(pattern):
                    ...
                    i += 1
                if all_matched: matches.append(s)
        except ValueError:
            # Not even the first item has matched, so we're done.
            return matches
...
def main(argv):
    if (len(argv) < 3):
        ...
        print "Usage: %s input_file chunk_size chunks" % argv[0]
        print "Final text will be about chunk_size * chunks words long."
        return -1
    fname = argv[1]
    chunk_size = int(argv[2])
    chunks = int(argv[3])
    print dissociated_press_from_file(fname, chunk_size, chunks)
